...
<?php include './BranchAdmin_page.css'; ?>
            ...
            *{
            ...
            .row-title{
                margin-top: 15px;
                box-shadow: 2px 2px #009999!important;
            }
            .row-title {
            margin:0px; 
            padding: 20px; 
        ...
        <div class="container-fluid allbranchContainer">
            <div class="row">
                ...
                <div class="col-sm-9 maincolumn">
                    ...
                    <?php
                    $sql = "select * from appointment where hiring_id in(select id from hiring where candidates_id in (select id from candidates where branches_id=" . $_SESSION['loggedinid'] . "))";
                    if (isset($_GET["status"])) {
                        $filterval = $_GET["status"];
                        $sql = "select * from appointment where appointment_status='$filterval' and hiring_id in(select id from hiring where candidates_id in (select id from candidates where branches_id=" . $_SESSION['loggedinid'] . "))";
                    }
                    $data = $conn->query($sql);
                            ...
                            <div class="col-sm-2">
                                <a href="../resume.php?candidate=<?php echo $candi["id"]; ?>"><img src="../images/profile/<?php echo $candi["dp"]; ?>" height="150" width="150" class="img-thumbnail img-responsive">
                                    <div class="column-text">
                                        <?php echo $candi["id"].":"; echo $candi["fname"]; ?> <?php echo $candi["lname"]; ?>
                                    </div>
